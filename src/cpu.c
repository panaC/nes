...
int clr_opcode(t_registers *reg, t_mem *memory, uint8_t op)
{
	...
	{
	case 0x18:
		debug_opcode_implied("CLC", op, (union u16){.value = 0});
		// clc
		reg->p.C = 0;
		break;

	case 0xd8:
		debug_opcode_implied("CLD", op, (union u16){.value = 0});
		// cld
		reg->p.D = 0;
		break;

	case 0x58:
		debug_opcode_implied("CLI", op, (union u16){.value = 0});
		// cli
		reg->p.I = 0;
		break;

	case 0xb8:
		debug_opcode_implied("CLV", op, (union u16){.value = 0});
		// clv
		reg->p.V = 0;
		break;
...
int dex_opcode(t_registers *reg, t_mem *memory, uint8_t op)
{
	...
	if (op == 0xca)
	{
		debug_opcode_implied("DEX", op, (union u16){.value = 0});
		;
		reg->x -= 1;
		reg->p.Z = reg->x == 0 ? 1 : 0;
		reg->p.N = reg->x < 0 ? 1 : 0;
...
int dey_opcode(t_registers *reg, t_mem *memory, uint8_t op)
{
	...
	if (op == 0x88)
	{
		debug_opcode_implied("DEY", op, (union u16){.value = 0});
		;
		reg->y -= 1;
		reg->p.Z = reg->y == 0 ? 1 : 0;
		reg->p.N = reg->y < 0 ? 1 : 0;
...
int inx_opcode(t_registers *reg, t_mem *memory, uint8_t op)
{
	...
	if (op == 0xe8)
	{
		debug_opcode_implied("INX", op, (union u16){.value = 0});
		;
		reg->x += 1;
		reg->p.Z = reg->x == 0 ? 1 : 0;
		reg->p.N = reg->x < 0 ? 1 : 0;
...
int iny_opcode(t_registers *reg, t_mem *memory, uint8_t op)
{
	...
	if (op == 0xc8)
	{
		debug_opcode_implied("INY", op, (union u16){.value = 0});
		;
		reg->y += 1;
		reg->p.Z = reg->y == 0 ? 1 : 0;
		reg->p.N = reg->y < 0 ? 1 : 0;
